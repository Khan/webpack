					...
					for (const chunk of chunks) {
						for (const file of chunk.files) {
							if (matchObject(file)) {
								...
								if (options.filename) {
									// options.filename is set to "[file].map[query]" when
									// using a devtool setting that outputs sourcemaps to
									// separate files.
									//
									// HACK(kevinb): we really should be using compilation.getPath
									// but it requires computing the hash of the source map string
									// and we want to avoid having to compute that becauses its
									// expensive and in our case unnecessary.
									const sourceMapFile = options.filename
										.replace("[file]", file)
										.replace("[query]", "");
