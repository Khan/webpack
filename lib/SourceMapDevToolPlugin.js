 ...
 *
 * @param {string} filename - file we're checking for
 * @param {object} compilation - current compilation object
 * @returns {boolean} - true if the file exists, otherwise false
 */
const doesOutputFileExist = (filename, compilation) => {
	const outputPath = path.join(compilation.options.output.path, filename);
					...
					for (const chunk of chunks) {
						for (const file of chunk.files) {
							if (matchObject(file)) {
								...
								if (options.skipExistingFiles && sourceMapFilename) {
									const filename = file;
									...
									if (placeholderMatch) {
										throw new Error(
											`${
												placeholderMatch[0]
											} not supported with options.skipExistingFiles`
										);
									}

									if (doesOutputFileExist(sourceMapFile, compilation)) {
										fileExists = true;
										// We add an empty asset for source map files that already
										// exist in the file system.  DisableEmitPlugin keeps us
										// from clobbering what's already on disk.
