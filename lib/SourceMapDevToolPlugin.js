...
/**
 * Get query string from filename.
 *
 * @param {string} filename - filename to check for query string
...
/**
 * Get the filename of the source map being generated.
 *
 * @param {object} compilation - compilation object
 * @param {string} sourceMapFilename - string template for output filename
 * @param {string} filename - relative path of output source map file
...
const getSourceMapFile = (
	...
	const query = getQuery(filename);

	const sourceMapFile = compilation.getPath(sourceMapFilename, {
		chunk,
		filename: fileContext ? path.relative(fileContext, filename) : filename,
		query,
		basename: basename(filename),
		contentHash
	});

	const placeholderMatch = /\[[^\]]+\]/.exec(sourceMapFile);
	if (placeholderMatch) {
		throw new Error(
			`${placeholderMatch[0]} not supported with options.skipExistingFiles`
		);
	}
...
/**
 * Determine whether or not to generate a source map file.
 *
 * @param {object} compilation - compilation object
 * @param {object} options - options object
 * @param {string} sourceMapFilename - string template for output filename
 * @param {string} file - relative path of output source map file
 * @param {object} chunk - chunk object
 * @returns {boolean} -
 */
const shouldGenerateSourceMapFile = (
	compilation,
	options,
	sourceMapFilename,
	file,
	...
	if (
		...
		options.shouldGenerateSourceMapFile
	) {
		const filename = file;

		const sourceMapFile = getSourceMapFile(
			compilation,
			sourceMapFilename,
			filename,
			chunk,
		...
		if (options.shouldGenerateSourceMapFile(sourceMapFile, compilation)) {
			return true;
		} else {
			compilation.assets[sourceMapFile] = new RawSource("");
			const outputPath = compilation.options.output.path;
			const targetPath = path.join(outputPath, filename);
			// We set the existsAt property to get Webpack to
			// skip outputting the file. Looking at the logic here:
			// https://github.com/webpack/webpack/blob/ccc7db79bab25ed3ef49435abc8866b63721b8b6/lib/Compiler.js#L330
			compilation.assets[sourceMapFile].existsAt = targetPath;
			return false;
		}
	}
	return true;
			...
			compilation.hooks.afterOptimizeChunkAssets.tap(
				...
				(context, chunks) => {
					...
					tasks.forEach((task, index) => {
						...
						if (typeof sourceMapFilename === "string") {
							const filename = file;
